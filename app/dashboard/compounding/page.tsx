...
import { TrendingUp, DollarSign, Target, PieChart, History, RotateCcw, Sliders, ChevronRight } from "lucide-react";
...
export default function CompoundingPage() {
  ...
  // Position size calc
  const slDistanceDollars = 10; // typical XAU/USD SL distance ~$10
  const riskAmount = parseFloat((balance * compounding.riskPercent / 100).toFixed(2));
  const suggestedLots = parseFloat((riskAmount / slDistanceDollars).toFixed(2));
  const potential1R = parseFloat((riskAmount * 1).toFixed(2));
  const potential2R = parseFloat((riskAmount * 2).toFixed(2));

  // Projections: next 10 cycles
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="space-y-3">
            <div>
              <div className="flex justify-between text-xs mb-1">
                ...
                <span className="font-mono font-bold text-accent-gold">{compounding.riskPercent}%</span>
              </div>
              <input
                type="range"
                min="1"
                max="20"
                step="0.5"
                value={compounding.riskPercent}
                onChange={e => updateCompounding({ riskPercent: parseFloat(e.target.value) })}
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="rounded-xl p-3 text-center" style={{ background: "rgba(0,230,118,0.06)", border: "1px solid rgba(0,230,118,0.15)" }}>
              <p className="text-[10px] text-text-muted uppercase">Lots (est.)</p>
              <p className="text-lg font-bold font-mono text-status-win">{suggestedLots.toFixed(2)}</p>
              <p className="text-[9px] text-text-muted mt-0.5">~${slDistanceDollars} SL</p>
            </div>
            <div className="rounded-xl p-3 text-center" style={{ background: "rgba(240,180,41,0.06)", border: "1px solid rgba(240,180,41,0.12)" }}>
